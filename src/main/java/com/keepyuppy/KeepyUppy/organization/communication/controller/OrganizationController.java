...
public class OrganizationController {
    ...
    public Long createTeam(Users users,
        ...
        return groupService.createTeam(users, createTeamRequest);
    }
    ...
    public TeamResponse getTeamById(@PathVariable Long id) {
        return groupService.getTeamById(id);
    }
    ...
    public List<TeamByUserIdResponse> getMyTeam(@PathVariable Long userId) {
        return groupService.getTeamByUser(userId);
    }

    @Operation(summary = "팀 소유자 일경우 팀 삭제")
    @PostMapping("/remove/{teamId}")
    public boolean removeTeam(@AuthenticationPrincipal Users users, @PathVariable Long teamId) {
        return groupService.removeTeam(users, teamId);
    }
    ...
    public void updateLinks(@PathVariable Long teamId , @RequestBody UpdateTeamLinks updateTeamLinks) {
        groupService.updateLinks(teamId,updateTeamLinks);
    }

    @Operation(summary = "로그인한 유저가 초대받은 응답 대기중인 팀 목록 조회")
    @GetMapping("/team/pending")
    public List<TeamByUserIdResponse> getPendingTeams(Users users) {
        return groupService.getPendingTeams(users.getId());
    }
}